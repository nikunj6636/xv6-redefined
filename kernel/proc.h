...
struct proc {
  ...
  // for waitx, ctime(FCFS scheduler)
  ...
  int queue_priority;  // priority_queue of process
  int in_queue;  // whether process is in queue or not
  int time_quantum;   // time quantum of the queue process is in
  int entry_time;     // entry_time in queue
  int qrtime[nmlfq];      // time spent in the queue
...
struct Queue{
  int head, tail; // tail points to last element in the queue, head first element
  struct proc *procs[NPROC]; // array of process
  int size;
};

typedef struct Queue Queue;

struct proc *top(Queue *q);
